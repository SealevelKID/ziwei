...
// 直接定義中州派四化表：祿、權、科、忌 (0=甲...9=癸)
const SI_HUA_TABLE = [
    ["廉貞", "破軍", "武曲", "太陽"], // 甲
    ["天機", "天梁", "紫微", "太陰"], // 乙
    ["天同", "天機", "文昌", "廉貞"], // 丙
    ["太陰", "天同", "天機", "巨門"], // 丁
    ["貪狼", "太陰", "右弼", "天機"], // 戊 (中州派：右弼化科)
    ["武曲", "貪狼", "天梁", "文曲"], // 己
    ["太陽", "武曲", "天同", "天相"], // 庚 (中州派：天相化忌)
    ["巨門", "太陽", "武曲", "文昌"], // 辛
    ["天梁", "紫微", "左輔", "武曲"], // 壬 (中州派：左輔化科)
    ["破軍", "巨門", "太陰", "貪狼"]  // 癸
];
...
// 更新監聽清單（移除 'sihuaVersion'）
['yearStem', 'yearBranch', 'birthMonth', 'birthDay', 'birthHour'].forEach(id => {
    const el = document.getElementById(id);
    if (el) {
        el.addEventListener('change', (e) => {
            const val = parseInt(e.target.value);
            if (id === 'yearStem') birthParams.yearStem = val;
            if (id === 'yearBranch') birthParams.yearBranch = val;
            if (id === 'birthMonth') birthParams.month = val;
            if (id === 'birthDay') birthParams.day = val;
            if (id === 'birthHour') birthParams.hour = val;
            drawBoard();
        });
    }
});
...
function drawSingleStar(targetPos, starName, color, countsObj, isMinor = false) {
    ...
    // 💡 關鍵精簡：直接從陣列抓取對應年干的四化名單
    const currentSihua = SI_HUA_TABLE[birthParams.yearStem];

    // 使用全名精確比對 (===)
    currentSihua.forEach((target, index) => {
        if (starName === target) { 
